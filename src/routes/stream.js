...
const { CACHE_TTL_SUCCESS, CACHE_TTL_ERROR, axiosConfig } = require('../config');
...
const streamHandler = async (req, res) => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), axiosConfig.timeout);
    const reqConfig = { ...axiosConfig, signal: controller.signal };
    ...
    try {
        ...
        if (title) {
            ...
            const pAc = scrapers.checkAfterCredits(title, year, reqConfig);
            const pMs = scrapers.checkMediaStinger(title, year, reqConfig);
            const pTmdb = scrapers.checkTmdb(id, metaRes.data?.meta?.moviedb_id, apiKey, reqConfig);
            const pWiki = scrapers.checkWikipedia(title, reqConfig);

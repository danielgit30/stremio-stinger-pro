...
async function getRelatedMovies(tmdbIdRaw, apiKey, reqConfig, imdbId) {
    ...
    // Await basedon.media concurrently or as fallback
    if (basedOnPromise) {
        ...
        if (basedOnRes && basedOnRes.data) {
            ...
            if (stream && stream.title) {
                ...
                if (match) {
                    ...
                    log(`[TMDB - Related] Exact Source Material from basedon.media prioritized: "${sourceMaterial}"`);
                }
            }
        } else {
    ...
    // Fallback to TMDB keywords if basedon.media did not provide sourceMaterial
    if (!sourceMaterial && tmdbSuccess && movie && movie.keywords && movie.keywords.keywords) {
    ...
    // Process franchise & collection details if TMDB succeeded
    if (tmdbSuccess && movie) {
        ...
        if (franchiseMatch) {
            ...
            if (franchiseMatch.keywordId) {
                ...
                try {
                    ...
                    log(`[TMDB - Related Warning] Failed to fetch collection details: ${err.message}`);
                }
            } else {
    ...
    return {
        ...
        sourceMaterial,
        collectionName,
        collectionUrl: collectionUrl || (tmdbId ? `https://www.themoviedb.org/movie/${tmdbId}` : null),
    };
}
...
async function getFranchiseNeighbors(keywordId, apiKey, reqConfig, targetReleaseDate, currentMovieId) {
    ...
    try {
        ...
        error(
            `[TMDB - Franchise Warning] Neighbor discover failed for keyword ID ${keywordId}: ${sanitizeError(err.message)}`
        );
    }
    return { prequel, sequel };
}
